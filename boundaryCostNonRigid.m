function [J,grad] = boundaryCostNonRigid(phi,points,normals,fixedImage,Fx,Fy)
...
Jr = abs(diff(phi));%gradient of transform
Jr = [Jr; phi(1,:) - phi(end,:)];
Jr = sum(Jr,2); %sum over dimensions
Jr = sum(Jr); %sum over boundary points

J = J + Jr;

gradJr = -imfilter(phi,[1 -2 1]);
